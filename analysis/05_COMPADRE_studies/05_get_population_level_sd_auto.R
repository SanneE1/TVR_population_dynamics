...
df$Tmean_control_0.5 <- NA
df$Tmean_Umatrix_0.5 <- NA
df$Sig_t_0.50 <- NA
...
for(sp in c(1:nrow(df))) {
  ...
  test1 <- df1 %>% group_by(sig.strength) %>% wilcox_test(lambda ~ type) %>% add_significance() %>% as.data.frame() %>% arrange(sig.strength)
  ...
  df[which(df$SpeciesAuthor == i & df$MatrixPopulation == j), 
     c("Sig_p_0.05", "Sig_p_0.25", "Sig_p_0.50", "Sig_p_1")] <- round(test1$p, digits = 3)
  df[which(df$SpeciesAuthor == i & df$MatrixPopulation == j),
  ...
  # Calculate average lambda's for each climate signal strength (p in manuscript) and
  # use wolcox to test for significant difference between control and MCD stoch. lambda's
  test2 <- df2 %>% group_by(sig.strength) %>% wilcox_test(lambda ~ type) %>% add_significance() %>% as.data.frame() %>% arrange(sig.strength)
  test2b <- df2 %>% group_by(sig.strength, type) %>% get_summary_stats(lambda) %>% 
    mutate(mean_type = paste("Tmean", type, sig.strength, sep = "_")) %>% 
    tibble::column_to_rownames(var = "mean_type")

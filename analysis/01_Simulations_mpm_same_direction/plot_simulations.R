...
### Load results
location <- "results/01_Simulations_mpm_same_directions/rds/"

auto <- lapply(list.files(location, pattern = "auto.RDS", full.names = T), readRDS)
cov <- lapply(list.files(location, pattern = "cov.RDS", full.names = T), readRDS)
lag <- lapply(list.files(location, pattern = "lag.RDS", full.names = T), readRDS)
lagpf <- lapply(list.files(location, pattern = "lagfp.RDS", full.names = T), readRDS)
...
## Save histograms of cell values  ---------- STILL NEEDS WORK ----------
output_cell <- 
pdf(paste("results/01_Simulations_mpm_same_directions/cell_value_plots/sigstrength", i, ".pdf"))

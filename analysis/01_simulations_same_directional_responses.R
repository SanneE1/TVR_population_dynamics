...
if(length(args)!=3) {
  stop("Wrong number of arguments provided", call.=F)
}
...
lag_p <- lapply(lag_clim,
                     function(x) st.lamb(mpm_df = lh_df,
                                         env_surv = x$lagged,
                                         env_growth = x$lagged,
...
write.csv(lag_df, file.path(output_dir, paste("mpm", i, taskID, "lagfp.csv", sep = "_")), row.names = F)
